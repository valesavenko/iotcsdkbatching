...
#define MESSAGE_COUNT 9 // determines the number of times the device tries to send a message to the IoT Hub in the cloud.
...
static void send_confirm_callback(IOTHUB_CLIENT_CONFIRMATION_RESULT result, void* userContextCallback)
{
    ...
    Serial.println("confirmation call back");//display same received Data back in serial monitor.
...
static void run_demo()
{
    ...
    // Turn Bathing ON
    bool setBatching = true;
    
    IoTHubDeviceClient_LL_SetOption(device_ll_handle, "Batching", &setBatching);

    // action phase of theprogram , sending messages to the IoT Hub in the cloud.
    do
    {
        if (messages_sent < MESSAGE_COUNT)
        {
            ...
            float humidity = 0.0;
             ...
             for (int i = 0; i < 9; ++i)
             {
                ...
                sprintf_s(date, sizeof(date), "2020-06-17T04:0%d:29.581Z", i);
                (void)IoTHubMessage_SetProperty(message_handle, "iothub-creation-time-utc", date);
                ...
                messages_sent++;
                Serial.println("Sending data");//display same received Data back in serial monitor.
                delay(1000);
             }
        }
        ...
        Serial.println("Do Work");//display same received Data back in serial monitor.
        Serial.println(messages_sent);//display same received Data back in serial monitor.

        IoTHubDeviceClient_LL_DoWork(device_ll_handle);
        Serial.println("finished doing work");//display same received Data back in serial monitor.
        delay(10000);
     ...
     Serial.println("destrory");//display same received Data back in serial monitor.
